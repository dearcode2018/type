 ...
 * CollectionSortTest
 */
public final class CollectionSortTest extends BaseTest {

	/**
	 * 集合排序
	 * 
	 * 1) List排序
	 * A. 实体实现 [可比较] 接口 0: 相等，-1: this小于other，1: this大于other
	 * 执行排序，使用Comparable接口方式 Collections.sort(list);
	 * 
	 * B. 构造比较器 	 升序: 0: 相等，-1: o1小于o2，1: o1大于o2 降序: 0: 相等，1: o1小于o2，-1: o1大于o2
		// 执行排序，使用Comparator比较器接口方式
		Collections.sort(list, new SortObjectComparator());
		
	 * C. 默认是使用实体类的Comparable接口去排序，若指定比较接口实现类 则按照比较器实现类来排序
	 * 
	 * 2) Set排序 TreeSet implements SortedSet
	 * A. 构造TreeSet, 作为构造参数传入集合参数，触发add方法回再次排序
	 * B. 构造TreeSet, 作为构造参数传入构造器对象，触发add方法回再次排序
	 * C. 一个空的TreeSet，每次add都会触发新的排序
	 * D. 默认是使用实体类的Comparable接口去排序，若指定比较接口实现类 则按照比较器实现类来排序
	 * 
	 * 3) Map排序
	 * A. 根据key排序 
	 * B.根据value排序 
	...
	public void testCollectionSort() {
		try {
			...
			log.error("testCollectionSort =====> ", e);
		}
	}
